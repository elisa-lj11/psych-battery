...
void setup() {
  ...
  // Power on display panel, then init GPIO — matches official Elecrow example order
  pinMode(7, OUTPUT);
  digitalWrite(7, HIGH);
  EPD_GPIOInit();
  ...
  // Clear screen first, then draw 100% on boot
  EPD_FastMode1Init();
  EPD_Display_Clear();
  EPD_Update();
...
void drawChargeBar(int pct) {
  ...
  if (isAlert) {
    ...
    int numW  = numDigits * 24;  // 24px per digit at font size 48
    int unitW = 16;              // "%" at font size 24
    int textX = BX + BW / 2 - (numW + unitW) / 2;
    int textY = BY + BH / 2 - 24;
    EPD_ShowNum(textX, textY, pct, numDigits, 48, WHITE);
    EPD_ShowString(textX + numW, textY + 12, "%", 24, WHITE);
    ...
    int numW  = numDigits * 24;  // 24px per digit at font size 48
    int unitW = 16;              // "%" at font size 24
    int textX = centerX - (numW + unitW) / 2;
    int textY = BY + BH / 2 - 24;
    EPD_ShowNum(textX, textY, pct, numDigits, 48, textColor);
    EPD_ShowString(textX + numW, textY + 12, "%", 24, textColor);
  }
}
